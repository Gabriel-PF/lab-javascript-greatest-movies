// Code is below the movies array.
...
function getAllDirectors(movies) {
  const allDirectiors = movies.map((movie) => {
    return movie.director;
  });

  return allDirectiors;
}
...
function scoresAverage(movies) {
  ...
  const correctObjects = movies.filter(function (item) {
  ...
  /* Got lost trying to use "toFixed" and "parseFloat"*/
}
...
function dramaMoviesScore(movies) {
  ...
  const totalSum = dramaMovies.reduce(function (previousValue, currentValue) {
   ...
   /* Same as interaction 3  */
}


// Iteration 5: Ordering by year - Order by year, ascending (in growing order)
function orderByYear(movies) {
  const newListOfMovies = [...movies];
